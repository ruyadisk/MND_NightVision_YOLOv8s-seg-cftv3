...
def train(cfg=DEFAULT_CFG, use_python=False, args=any):
    
    model = args.model        
    data = args.data
    path = os.getcwd() + '/runs/segment'
    print(model)
    train_args = dict(model=model, data=data, batch=args.batch, epochs=args.epoch, project=path)
    
    if use_python:
        from ultralytics import YOLO
        YOLO(model).train(**train_args)
    else: 
        trainer = SegmentationTrainer(overrides=train_args)
        trainer.train()

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    
    parser.add_argument("--model", default="./ultralytics/models/yolov8s-seg-cftv3.yaml" ,type=str, help="pretrained path or model yaml path")
    parser.add_argument("--data", default="", type=str, help="data yaml path")
    parser.add_argument("--batch", default=16, type=int, help="data yaml path")
    parser.add_argument("--epoch", default=275, type=int, help="data yaml path")
