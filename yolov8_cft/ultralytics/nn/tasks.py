...
from ultralytics.nn.modules import (C1, C2, C3, C3TR, SPP, SPPF, Bottleneck, BottleneckCSP, C2f, C3Ghost, C3x, Classify,
                                    ...
                                    GhostBottleneck, GhostConv, Segment, Add, GPT, AddT)
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolov8n.yaml', ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        if isinstance(m, (Detect, Segment)):
            ...
            forward = lambda x: self.forward(x,x)[0] if isinstance(m, Segment) else self.forward(x,x) # add
    ...
    def forward(self, x, x2, augment=False, profile=False, visualize=False):
        ...
        return self._forward_once(x, x2, profile, visualize) # add
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if f == -11 and m is Conv: # two input
            c1, c2 = 3, 64
