...
class Add(nn.Module):
    def __init__(self,skip): # skip not to use
        super().__init__()
...
class GPT(nn.Module):
    ...
    def __init__(self, d_model, h=8, block_exp=4,
        ...
        self.trans_blocks = nn.Sequential(*[myTransformerBlock(d_model, d_k, d_v, h, block_exp, attn_pdrop, resid_pdrop)
                                            for layer in range(n_layer)]) # transformer
        self.ln_f = nn.LayerNorm(self.n_embd) # decoder head
        self.drop = nn.Dropout(embd_pdrop) # regularization
        self.avgpool = nn.AdaptiveAvgPool2d((self.vert_anchors, self.horz_anchors)) # avgpool
