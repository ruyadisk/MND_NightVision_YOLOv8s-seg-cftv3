...
class BasePredictor:
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None):
        ...
        project = os.getcwd() + '/runs/predict'
    ...
    def stream_inference(self, source=None, model=None):
        ...
        for batch in self.dataset:
            ...
            self.batch = batch
            # path, im, im2, im0s, vid_cap, s = batch # revise
            path, path2, im, im2, im0s, im0s_2, vid_cap, s = batch # revise2
            visualize = increment_path(self.save_dir / Path(path).stem, mkdir=True) if self.args.visualize else False

            # preprocess
            with self.dt[0]:
                im = self.preprocess(im)
                im2 = self.preprocess(im2) # add
                if len(im.shape) == 3:
            ...
            for i in range(n):
                ...
                p2, im0_2 = (path2[i], im0s_2[i].copy()) if self.source_type.webcam or self.source_type.from_img \
                    else (path2, im0s_2.copy()) # revise2
                p = Path(p)

                if self.args.verbose or self.args.save or self.args.save_txt or self.args.show:
                    # s += self.write_results(i, self.results, (p, im, im0))
                    s += self.write_results(i, self.results, (p, im, im0, im0_2)) # revise2
