...
class LoadImages:
    ...
    def __init__(self, path, imgsz=640, stride=32, auto=True, transforms=None, vid_stride=1):
        ...
        if path == os.getcwd() + '/ultralytics/assets/bus.jpg':
            path1 = path
            path2 = path
        else:
            path1 = path + '/night'
            path2 = path + '/thermal'
        files = []
        for p in sorted(path1) if isinstance(path1, (list, tuple)) else [path1]: # revise
        ...
        files2 = [] 
        for p in sorted(path2) if isinstance(path2, (list, tuple)) else [path2]:
            p = str(Path(p).resolve())
            if '*' in p:
                files2.extend(sorted(glob.glob(p, recursive=True)))  # glob
            elif os.path.isdir(p):
                files2.extend(sorted(glob.glob(os.path.join(p, '*.*'))))  # dir
            elif os.path.isfile(p):
                files2.append(p)  # files
            else:
                raise FileNotFoundError(f'{p} does not exist')
        ...
        if any(videos):
            ...
            self._new_video(videos[0],videos2[0])  # new video # revise(one parameter to two parameter)
        else:
            self.cap = None
        if self.nf == 0:
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            success2, im0_2 = self.cap2.retrieve()
            while not success or not success2:
                ...
                path = self.files[self.count]
                path2 = self.files2[self.count]
                self._new_video(path, path2) 
                success, im0 = self.cap.read()
                success2, im0_2 = self.cap2.read()
            ...
            im0_2 = cv2.imread(path2) 
            if im0 is None or im0_2 is None:
        ...
        if self.transforms:
            ...
            im = im.transpose((2, 0, 1))[::-1]  # HWC to CHW, BGR to RGB
            im = np.ascontiguousarray(im)  # contiguous
            im2 = LetterBox(self.imgsz, self.auto, stride=self.stride)(image=im0_2)
            im2 = im2.transpose((2, 0, 1))[::-1] 
            im2 = np.ascontiguousarray(im2) 

        return path, path2, im, im2, im0, im0_2, self.cap, s 

    def _new_video(self, path, path2): # revise(one parameter to two parameter)
