...
class Mosaic(BaseMixTransform):
    ...
    def _mix_transform(self, labels):
        ...
        for i in range(4):
            ...
            img2 = labels_patch['img2'] # add
            h, w = labels_patch.pop('resized_shape')

            # place img in img4
            if i == 0:  # top left
                img4 = np.full((s * 2, s * 2, img.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
                img42 = np.copy(img4) # add
            ...
            img42[y1a:y2a, x1a:x2a] = img2[y1b:y2b, x1b:x2b] # add
            padw = x1a - x1b
            padh = y1a - y1b
...
class MixUp(BaseMixTransform):
    ...
    def _mix_transform(self, labels):
        ...
        labels['img2'] = (labels['img2'] * r + labels2['img2'] * (1 - r)).astype(np.uint8) # add
...
class RandomPerspective:
    ...
    def affine_transform(self, img, img2, border):
        ...
        if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
            if self.perspective:
                ...
                img2 = cv2.warpPerspective(img2, M, dsize=self.size, borderValue=(114, 114, 114)) # add
            else:  # affine
                img = cv2.warpAffine(img, M[:2], dsize=self.size, borderValue=(114, 114, 114))
                img2 = cv2.warpAffine(img2, M[:2], dsize=self.size, borderValue=(114, 114, 114)) # add
        return img, img2, M, s
    ...
    def __call__(self, labels):
        ...
        img, img2, M, scale = self.affine_transform(img, img2, border) # revise
...
class RandomHSV:
    ...
    def __call__(self, labels):
        ...
        if self.hgain or self.sgain or self.vgain:
            ...
            hue2, sat2, val2 = cv2.split(cv2.cvtColor(img2, cv2.COLOR_BGR2HSV)) # add
            ...
            im_hsv2 = cv2.merge((cv2.LUT(hue2, lut_hue), cv2.LUT(sat2, lut_sat), cv2.LUT(val2, lut_val))) # add
            cv2.cvtColor(im_hsv, cv2.COLOR_HSV2BGR, dst=img)  # no return needed
            cv2.cvtColor(im_hsv2, cv2.COLOR_HSV2BGR, dst=img2) # add
        return labels
...
class RandomFlip:
    ...
    def __call__(self, labels):
        ...
        if self.direction == 'vertical' and random.random() < self.p:
            img = np.flipud(img)
            img2 = np.flipud(img2) # add
            instances.flipud(h)
        if self.direction == 'horizontal' and random.random() < self.p:
            img = np.fliplr(img)
            img2 = np.fliplr(img2) # add
            instances.fliplr(w)
        labels['img'] = np.ascontiguousarray(img)
        labels['img2'] = np.ascontiguousarray(img2) # add
...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        ...
        img2 = labels.get('img2') if image is None else image # add
        ...
        if shape[::-1] != new_unpad:  # resize
            ...
            img2 = cv2.resize(img2, new_unpad, interpolation=cv2.INTER_LINEAR) # add
        top, bottom = int(round(dh - 0.1)), int(round(dh + 0.1))
        left, right = int(round(dw - 0.1)), int(round(dw + 0.1))
        img = cv2.copyMakeBorder(img, top, bottom, left, right, cv2.BORDER_CONSTANT,
                                 value=(114, 114, 114))  # add border
        img2 = cv2.copyMakeBorder(img2, top, bottom, left, right, cv2.BORDER_CONSTANT,
                                 value=(114, 114, 114))  # add                  
...
class CopyPaste:
    ...
    def __call__(self, labels):
        ...
        if self.p and len(instances.segments):
            ...
            im_new2 = np.zeros(im2.shape, np.uint8) # add
            ...
            for j in random.sample(list(indexes), k=round(self.p * n)):
                ...
                cv2.drawContours(im_new2, instances.segments[[j]].astype(np.int32), -1, (1, 1, 1), cv2.FILLED) # add

            result = cv2.flip(im, 1)  # augment segments (flip left-right)
            result2 = cv2.flip(im2, 1) # add
            i = cv2.flip(im_new, 1).astype(bool)
            i2 = cv2.flip(im_new2, 1).astype(bool) # add
            im[i] = result[i]  # cv2.imwrite('debug.jpg', im)  # debug
            im2[i2] = result2[i2] # add
    ...
    def __call__(self, labels):
        ...
        if len(cls):
            ...
            if self.transform and random.random() < self.p:
                ...
                new2 = self.transform(image=im2, bboxes=bboxes, class_labels=cls) # add
                if len(new['class_labels']) > 0:  # skip update if no bbox in new im
                    labels['img'] = new['image']
                    labels['img2'] = new2['image'] # add
...
class Format:
    ...
    def __call__(self, labels):
        ...
        labels['img2'] = self._format_img(img2) # add
